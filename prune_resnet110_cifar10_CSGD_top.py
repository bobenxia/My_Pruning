...
# from model.cifar.resnet import resnet110
...
def train_with_csgd(model, model_name, train_loader, test_loader, is_resume, model_save_path):
    # -------------------- 根据模型选择的一些超参 -------------------
    deps = rc_origin_deps_flattened(18)  # resnet110 的 通道数量
    origin_deps_flattened = rc_origin_deps_flattened(18)
    internal_kernel_idexs = rc_internal_layers(18)
    ...
    # }
    pacesetter_dict = rc_pacesetter_dict(18)
    # --------------------------- done ----------------------------
...
def main():
    ...
    tos_model_save_path = '/tos/save_data/my_pruning_save_data/log_and_model/' + 'SGD_CAWR_test2_600epoch/' +'0.80-'+ TIMESTAMP
    tos_tensorboard_log_path = '/tos/save_data/my_pruning_save_data/log_and_model/' + 'SGD_CAWR_test2_600epoch/' +'0.80-'+ TIMESTAMP
    ...
    if args.mode == 'train':
        ...
        # model = torch.load(previous_ckpt, map_location=torch.device("cpu"))
        model_name = "resnet110-CSGD-finetune"
        ...
        # # ckpt = 'save/train_and_prune/2022-02-16T00-42-55/resnet110-finetune-round1.pth'
        ckpt = "save/train_and_prune/2022-02-22T10-38-38/resnet110-CSGD-finetune-round1.pth"
        ...
        #         # model = resnet110(num_classes=10)
        #         # load_hdf5(model, hdf5_file)
        #         ckpt = f"/tos/save_data/my_pruning_save_data/log_and_model/SGD_CAWR_{l}_{n}_600epoch/resnet110-CSGD-finetune-round1.pth"
        #         print("Load model from %s" % (ckpt))
        #         # # need load model to cpu, avoid computing model GPU memory errors
        #         model = torch.load(ckpt, map_location=torch.device("cpu"))
