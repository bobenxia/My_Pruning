...
LRSchedule = namedtuple(
    'LRSchedule',
    ['base_lr', 'max_epochs', 'lr_epoch_boundaries', 'lr_decay_factor', 'linear_final_lr', 'cosine_minimum'])

VGG_ORIGIN_DEPS = [64, 64, 128, 128, 256, 256, 256, 512, 512, 512, 512, 512, 512]
CFQK_ORIGIN_DEPS = np.array([32, 32, 64], dtype=np.int32)

MI1_ORIGIN_DEPS = np.array([
    32, 32, 64, 64, 128, 128, 128, 128, 256, 256, 256, 256, 512, 512, 512, 512, 512, 512, 512, 512, 512, 512, 512, 512,
    1024, 1024, 1024
])
...
#   3, 4, 6, 3
RESNET50_PW_LAYERS = [
    1, 3, 4, 6, 7, 9, 10, 11, 13, 14, 16, 17, 19, 20, 22, 23, 24, 26, 27, 29, 30, 32, 33, 35, 36, 38, 39, 41, 42, 43,
    45, 46, 48, 49, 51
]
...
RESNET50_ORIGIN_DEPS = [64, [[64, 64, 256]] * 3, [[128, 128, 512]] * 4, [[256, 256, 1024]] * 6, [[512, 512, 2048]] * 3]
...
RESNET50_ORIGIN_DEPS_FLATTENED = [
    64, 64, 64, 256, 256, 64, 64, 256, 64, 64, 256, 128, 128, 512, 512, 128, 128, 512, 128, 128, 512, 128, 128, 512,
    256, 256, 1024, 1024, 256, 256, 1024, 256, 256, 1024, 256, 256, 1024, 256, 256, 1024, 256, 256, 1024, 512, 512,
...
def resnet_bottleneck_origin_deps_converted(res_n):
    ...
    return [
        64, [[64, 64, 256]] * num_blocks[0], [[128, 128, 512]] * num_blocks[1], [[256, 256, 1024]] * num_blocks[2],
...
def _resnet_bottlenck_first_internal_layer_idx_of_stage(num_blocks):
    return [
        2, 3 + num_blocks[0] * 3, 4 + (num_blocks[0] + num_blocks[1]) * 3,
        5 + (num_blocks[0] + num_blocks[1] + num_blocks[2]) * 3
    ]
...
def convert_resnet_bottleneck_deps(deps):
    ...
    for stage_idx in range(4):
        tmp = []
        assert deps[start_layer_idx_of_stage[stage_idx] - 1] == deps[start_layer_idx_of_stage[stage_idx] +
                                                                     2]  # check the proj layer deps
        for i in range(num_blocks[stage_idx]):
            tmp.append([
                deps[start_layer_idx_of_stage[stage_idx] + i * 3],
                deps[start_layer_idx_of_stage[stage_idx] + 1 + i * 3],
                deps[start_layer_idx_of_stage[stage_idx] + 2 + i * 3]
            ])
        d.append(tmp)
