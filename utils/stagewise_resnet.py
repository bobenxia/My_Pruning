...
from utils.constant import RESNET50_ORIGIN_DEPS_FLATTENED, resnet_bottleneck_origin_deps_flattened, rc_convert_flattened_deps, rc_origin_deps_flattened
import torch
import numpy as np
...
def create_SRC56(deps, builder):
    return SRCNet(block_counts=[9, 9, 9], num_classes=10, builder=builder, deps=deps)

def create_SRC110(deps, builder):
    return SRCNet(block_counts=[18, 18, 18], num_classes=10, builder=builder, deps=deps)

def create_SRC164(deps, builder):
    return SRCNet(block_counts=[27, 27, 27], num_classes=10, builder=builder, deps=deps)